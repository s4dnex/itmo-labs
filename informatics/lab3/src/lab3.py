...
texts = [
    ...
    # 2
    "После долгого рабочего дня я встретил свою подругу Марию. Она рассказала, что только что вернулась из путешествия по Европе с Петровой-Ковалевой М.Н., которая на самом деле была её соседкой.",
    # Answer:
    # Петровой-Ковалевой
...
pattern = re.compile(r"((?:[A-ZА-Я][a-zа-я]*){1}(?:-[A-ZА-Я][a-zа-я]*)?)\s[A-ZА-Я]\.(?:\b|\s)[A-ZА-Я]\.")
for i, t in enumerate(texts):
    surnames = pattern.findall(t)
    surnames.sort()
    print(f'{i + 1}.')
    for s in surnames:
        print(s)
